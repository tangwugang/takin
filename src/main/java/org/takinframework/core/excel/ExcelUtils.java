...
public class ExcelUtils {
	...
	static{
		xlsPath = ResourceUtil.getConfig("excel.xls");
	}
	  ...
	  public static void export(ServletContext ctx, String config, Object context, OutputStream out) throws ExcelException {
	    try {
	      HSSFWorkbook wb = WorkbookUtils.openWorkbook(ctx, xlsPath+config);
	   ...
	   * @throws ExcelException
	   */
	  public static void export(ServletContext ctx, String config, int[] sheetIndexs, OutputStream out) throws ExcelException {
		  try {
			  HSSFWorkbook wb = WorkbookUtils.openWorkbook(ctx, xlsPath+config);
			  int sheetCount = wb.getNumberOfSheets();
		      for (int sheetIndex = 0; sheetIndex < sheetCount; sheetIndex++) {
		        HSSFSheet sheet = wb.getSheetAt(sheetIndex);
		        ExcelParser.parse(getContext(), sheet, sheet.getFirstRowNum(), sheet.getLastRowNum());
		      }
		      wb.write(out);
		  } catch (Exception e) {
			  e.printStackTrace();
